...
// INCLUDES
// ================================================================
require_once '/var/www/html/lib/presto.php';
require_once '/var/www/html/lib/ssw.php';

$dominio        = strtoupper($argv[1]);
$data_ini       = $argv[2];
$data_fin       = $argv[3];
$periodo_coleta = $argv[4];

if ($data_ini == '')
{
  $data_ini = date ('dmy', strtotime ('-3 days'));
  $data_fin = date ('dmy');
}
...
$query = "SELECT domain FROM domains WHERE domain NOT IN ('XXX','EVI') AND is_active = true";

if ($dominio != '') $query .= " AND domain = '$dominio'";

$result_domains = sql ($g_sql, $query);

while ($row_domains = pg_fetch_array($result_domains))
{
  $dominio = $row_domains['domain'];
  ...
  try {
    ...
    if ($periodo_coleta == 'S')
      sql ($g_sql, "DELETE FROM $dominio" . "_coleta WHERE data_limite BETWEEN '$data_ini_fmt' AND '$data_fin_fmt'");
    else
      sql ($g_sql, "DELETE FROM $dominio" . "_coleta WHERE data_inclusao BETWEEN '$data_ini_fmt' AND '$data_fin_fmt'");
