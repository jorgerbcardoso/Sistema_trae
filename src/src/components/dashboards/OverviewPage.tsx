...
export function OverviewPage({ viewMode = 'GERAL', domainModalidade = 'CARGAS', period }: OverviewPageProps) {
  ...
  // ✅ CORREÇÃO DEFINITIVA: Adicionar IDs únicos com timestamp para evitar keys duplicadas no Recharts
  const uniqueTimestamp = Date.now();
  const monthlyDataWithKeys = monthlyData.map((item, index) => ({
    ...item,
    id: `month-${uniqueTimestamp}-${index}`,
    key: `month-${uniqueTimestamp}-${index}`
  }));
  
  const unitDataWithKeys = unitData.map((item, index) => ({
    ...item,
    id: `unit-${uniqueTimestamp}-${index}`,
    key: `unit-${uniqueTimestamp}-${index}`
  }));
    ...
    <main className="container mx-auto px-3 md:px-6 py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlyDataWithKeys}>
                ...
                />
                <Legend />
                <Line key="line-receita" type="monotone" dataKey="receita" stroke="#3b82f6" strokeWidth={2} name="Receita" dot={{ r: 4 }} activeDot={{ r: 6 }} />
                <Line key="line-despesas" type="monotone" dataKey="despesas" stroke="#ef4444" strokeWidth={2} name="Despesas" dot={{ r: 4 }} activeDot={{ r: 6 }} />
                <Line key="line-lucro" type="monotone" dataKey="lucro" stroke="#10b981" strokeWidth={2} name="Lucro" dot={{ r: 4 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={unitDataWithKeys} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" className="dark:stroke-slate-700" />
