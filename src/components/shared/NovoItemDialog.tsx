...
import { Loader2, CheckCircle, X, Package } from 'lucide-react';
...
export function NovoItemDialog({ open, onOpenChange, onItemCriado }: NovoItemDialogProps) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const data = await apiFetch(`${ENVIRONMENT.apiBaseUrl}/estoque/create_item_with_position.php`, {
        method: 'POST',
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Preencha os dados do novo item. Ele será criado e automaticamente adicionado ao estoque selecionado na posição PSO/1/1.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-2 border-t pt-4">
            ...
            <p className="text-xs text-gray-500">
              O item será criado na posição <strong>PSO/1/1</strong> do estoque selecionado
            </p>
          </div>
        </div>
