...
export function FilterSelectUnidadeOrdered({ value, onChange, disabled = false }: FilterSelectUnidadeOrderedProps) {
    ...
    <Popover open={disabled ? false : open} onOpenChange={disabled ? undefined : setOpen}>
      <PopoverTrigger asChild>
        <Button 
          variant="outline" 
          disabled={disabled}
          className="w-full justify-between text-left dark:bg-slate-800 dark:border-slate-700 h-auto min-h-[40px]"
        >
          ...
          <div className="flex items-center gap-1 ml-2 shrink-0">
            ...
            {!disabled && <Search className="h-4 w-4 opacity-50" />}
          </div>
        </Button>
      </PopoverTrigger>
      {!disabled && (
